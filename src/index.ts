...
class NetworkDriver {
    ...
    private async executeCommands(orgConfig: any, orderers: any[], artifactsDir: string, userCredPath: string, useCa: boolean): Promise<number> {
        ...
        if (rslt) {
            ...
            this.logger.error("Client initialization failure")
        }
        return new Promise<number>(resolve => { resolve(0) })
    }

    public async fabricClientFlowWithCert(): Promise<number> {
        this.logger.info("Running now....fabricClientFlowWithCert")
        let output = 0
        if (process.argv.length >= 4) {
            let org = process.argv[2]
            let command = process.argv[3]
            ...
            output = await this.executeCommands(orgConfig, orderers, "../artifacts", "../../user_cred_map_with_cert.json", false)
        }
        return new Promise<number>(resolve => { resolve(output) })
    }
    public async fabricClientFlowWithCA(): Promise<number> {
        this.logger.info("Running now....fabricClientFlowWithCert")
        let output = 0
        if (process.argv.length >= 4) {
            let org = process.argv[2]
            ...
            let orderers: object[] = networkConfig['orderer']
            var orgConfig = networkConfig[org]
            output = await this.executeCommands(orgConfig, orderers, "../artifacts", "../../user_cred_map_with_secret.json", true)
        }
        return new Promise<number>(resolve => { resolve(output) })
    }
    ...
    public async run(): Promise<number> {
        ...
        //await this.fabricClientFlowWithCert();
        await this.fabricClientFlowWithCA()
        return new Promise<number>(resolve => {
            resolve(0);
        });
